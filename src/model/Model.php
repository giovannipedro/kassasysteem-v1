...
class Model
{
    ...
    public function update(array $data, array $conditions): bool
    {
        ...
        foreach ($conditions as $column => $value) {
            $conditionData[] = "$column = :$column";
        }

        $updateQuery = "UPDATE $table SET " . implode(", ", $updateData) . " WHERE " . implode(" AND ", $conditionData);

        $bindings = array_merge($data, $conditions);

        return $this->db->execute($updateQuery, $bindings);
    }
}
